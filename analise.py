...
# Data Cleaning
def clean_text(text):
    text = re.sub("[^a-zA-Z]", " ", text)  # Remove tudo exceto letras
    text = ' '.join(text.split())  # Remove espaços em branco extras
...
def calculate_difference(original_text, corrected_text):
    ...
    difference_percentage = (difference / len(original_words))
...
def predict_fake_news_probability(text, model, vectorizer, difference_percentage):
    ...
    fake_news_probability = probability[0][1] * weight
